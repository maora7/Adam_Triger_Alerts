...
def setup_logging():
    """Setup logging with rotation - 200MB per file, keep 10 backups"""
    log_formatter = logging.Formatter("%(asctime)s - %(message)s")
    
    # Create rotating file handler
    # maxBytes = 200MB, backupCount = 10 files
    rotating_handler = logging.handlers.RotatingFileHandler(
        filename="adam_advantech_log.txt",
        maxBytes=200 * 1024 * 1024,  # 200MB in bytes
...
def get_config_path():
    """Get the config file path, handling both development and PyInstaller contexts"""
...
def load_config(mqtt_client=None):
    ...
    try:
        ...
        # Always reload if config_mtime is None (first load) or if file time changed
        if config_mtime is None or config_mtime != mtime:
            ...
            # Only update if we successfully loaded the config
            config = new_config
            config_mtime = mtime
            ...
            mandatory_keys = ["ADAM_MQTT", "ACTIVE_THRESHOLD_BY_SECONDS", "DEVICE_IP_MAP", "MQTT_TOPICS"]
            ...
            # Check if ADAM_MQTT mode changed
            if old_config and old_config.get("ADAM_MQTT", "").upper() != config.get("ADAM_MQTT", "").upper():
                logging.info(f"ADAM_MQTT mode changed - switching modes")
                print(f"ADAM_MQTT mode changed - switching modes")
...
monitor_thread = None  # Track monitor thread

# New: Store last known DI states for MQTT mode
last_di_states = {}  # {device_ip: {di1: bool, di2: bool, ...}}


def check_di_thresholds():
    """Check if any DI has been active longer than threshold and log if needed"""
    global di_active_since, logged_events
    
    ACTIVE_THRESHOLD_BY_SECONDS = config.get("ACTIVE_THRESHOLD_BY_SECONDS", 5)
    current_time = time.time()
    
    for key in list(di_active_since.keys()):
        if key not in logged_events:
            active_duration = current_time - di_active_since[key]
            if active_duration >= ACTIVE_THRESHOLD_BY_SECONDS:
                device_ip, channel = key
                if current_mode == "MQTT":
                    # For MQTT, channel format is "di1", "di2", etc.
                    channel_num = int(channel[2:]) - 1  # Convert "di1" to 0, "di2" to 1, etc.
                    msg = f"ERROR!!!!! MQTT {device_ip} - DI{channel_num} active for {ACTIVE_THRESHOLD_BY_SECONDS} seconds"
                else:
                    # For Modbus, channel is already a number
                    msg = f"ERROR!!!!! Modbus {device_ip} - DI{channel} active for {ACTIVE_THRESHOLD_BY_SECONDS} seconds"
...
def periodic_di_checker():
    """Periodic checker for DI states - runs in MQTT mode to handle brokers that only send on change"""
    global stop_current_mode
    
    CHECK_INTERVAL = 1  # Check every second
    
    while not stop_current_mode:
        if current_mode == "MQTT":
            # Only check in MQTT mode, Modbus mode handles this internally
            check_di_thresholds()
...
def adam_modbus_monitor():
    ...
    while not stop_current_mode:
        # Check config every iteration - if it fails, stop the monitor
        if not load_config():
            logging.error("Failed to load config during Modbus monitor. Stopping monitor.")
            print("Failed to load config during Modbus monitor. Stopping monitor.")
            stop_current_mode = True
            break
...
def mqtt_connection_monitor(client):
    ...
    while not stop_current_mode:
        ...
        # Check connection every 10 seconds, but only if we've been connected before
        if current_time - last_connection_check >= 10:
            last_connection_check = current_time
            
            # Only log connection down if we were previously connected
            if not mqtt_connected and hasattr(client, '_connected_once'):
                logging.error("MQTT broker connection is down")
                print("MQTT broker connection is down")
        
        # Check for configuration changes every 3 seconds
        try:
            # If config loading fails, stop the monitor
            if not load_config(client):
                logging.error("Config file missing or invalid - stopping MQTT monitor")
                print("Config file missing or invalid - stopping MQTT monitor")
                stop_current_mode = True
                break
...
def adam_mqtt_monitor():
    global mqtt_connected, stop_current_mode, current_mode, monitor_thread, last_di_states
    current_mode = "MQTT"
    stop_current_mode = False
    last_di_states = {}  # Reset DI states when starting MQTT monitor
    
    # Define callback functions that work with both API versions
    def on_connect(client, userdata, flags, rc, properties=None):
        global mqtt_connected
        if rc == 0:
            mqtt_connected = True
            client._connected_once = True  # Mark that we've connected at least once
    ...
    def on_message(client, userdata, msg):
        global di_active_since, logged_events, last_di_states
        ...
        try:
            ...
            if device_ip not in last_di_states:
                last_di_states[device_ip] = {}

            # Process DI channels 1-6 (di1 to di6 in JSON)
            for ch in range(1, 7):
                di_key = f"di{ch}"
                key = (device_ip, di_key)
                current_state = data.get(di_key, False)
                
                # Store the current state
                last_di_states[device_ip][di_key] = current_state
                
                if current_state:
                    # DI is active
                    if key not in di_active_since:
                        di_active_since[key] = time.time()
                        logging.debug(f"DI{ch-1} on {device_ip} became active")
                    # Note: Threshold checking is now handled by periodic_di_checker()
                else:
                    # DI is inactive
                    if key in di_active_since:
                        del di_active_since[key]
                        logging.debug(f"DI{ch-1} on {device_ip} became inactive")
                    if key in logged_events:
                        logged_events.remove(key)
    ...
    # Try to create client with newer API first, fall back to older API if needed
    try:
        # Try VERSION2 first (newer versions)
        client = mqtt.Client(callback_api_version=mqtt.CallbackAPIVersion.VERSION2)
        logging.info("Using MQTT Callback API VERSION2")
        print("Using MQTT Callback API VERSION2")
    except (AttributeError, TypeError):
        # Fall back to older API (VERSION1 or no version specified)
        client = mqtt.Client()
        logging.info("Using MQTT Callback API VERSION1 (legacy)")
        print("Using MQTT Callback API VERSION1 (legacy)")
    ...
    # Check if config can be loaded before starting
    if not load_config():
        logging.error("Cannot start MQTT monitor - config file missing or invalid")
        print("Cannot start MQTT monitor - config file missing or invalid")
        return
    ...
    # Start periodic DI checker thread
    di_checker_thread = threading.Thread(target=periodic_di_checker, daemon=True)
    di_checker_thread.start()
    logging.info("Started periodic DI checker thread")
    print("Started periodic DI checker thread")
    ...
    while not stop_current_mode:
        try:
            ...
            if not load_config():
                logging.error("Config file missing during MQTT connection attempt")
                print("Config file missing during MQTT connection attempt")
                break
    ...
    # Wait for DI checker thread to finish
    if di_checker_thread and di_checker_thread.is_alive():
        di_checker_thread.join(timeout=2)


if __name__ == "__main__":
    if not load_config():
        logging.error("Initial config load failed, exiting.")
        print("Initial config load failed, exiting.")
        exit(1)

    # Log startup message
    logging.info("=== ADAM Advantech Monitor Started ===")
    print("=== ADAM Advantech Monitor Started ===")

    # Main loop to handle mode switching
    while True:
        stop_current_mode = False
        
        # Check config before each iteration
        if not load_config():
            logging.error("Config file missing or invalid, exiting.")
            print("Config file missing or invalid, exiting.")
            break
        
        if config["ADAM_MQTT"].upper() == "TRUE":
        ...
        if not load_config():
            ...
            print("Failed to reload config, exiting.")
            break